export class ImageResize {
    ...
    handleDrag(evt) {
      ...
      if (this.dragBox === this.boxes[0] || this.dragBox === this.boxes[3]) {
        ...
        this.img.width = Math.round(
          this.preDragWidth - evt.clientX - this.dragStartX
        );
      } else {
    ...
    setCursor(value) {
      [document.body, this.img, this.quill.root].forEach(
        (el) => (el.style.cursor = value)
      );
    }
    ...
    positionSizeDisplay(rect) {
      ...
      if (size[0] > 120 && size[1] > 30) {
        ...
        this.extend(this.display.style, {
          left:
            Math.round(
              rect.left + rect.width + window.pageXOffset - dispRect.width - 8
            ) + "px",
          top:
            Math.round(
              rect.top + rect.height + window.pageYOffset - dispRect.height - 8
            ) + "px"
        });
      } else {
        ...
        this.extend(this.display.style, {
          left:
            Math.round(rect.left + rect.width + window.pageXOffset + 8) + "px",
